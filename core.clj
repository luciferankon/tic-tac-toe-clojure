...
(defn over?
  ...
  (cond
    ...
    :always (get-state :inplay "")))

(defn place-move
  [game move player]
  (let [game-with-move (->> move
                            (update player :moves conj)
                            (assoc game :current))]
...
(defn start-game []
  (loop [game {:current {:moves #{} :symbol :x} :opponent {:moves #{} :symbol :o}}
         move (read)]
    (let [updated-game (place-move game move (game :current))]
      (print-board updated-game)
      (if (game-over? updated-game)
        (println (updated-game :result))
        (recur (change-round updated-game) (read))))))
